...
export default function LetterCard({ letter }: { letter: PublicLetter }) {
  return (
    <article className="rounded-2xl border border-black/5 bg-white/60 p-7 shadow-sm dark:border-white/10 dark:bg-white/5 sm:p-9">
      <p className="whitespace-pre-line font-serif text-xl leading-[1.75] tracking-[0.01em]">
        {letter.content}
      </p>

      {letter.author_alias ? (
        <p className="mt-6 font-serif text-base italic opacity-70">
          — {letter.author_alias}
        </p>
      ) : (
        <p className="mt-6 font-serif text-base italic opacity-50">— anonym</p>
      )}
